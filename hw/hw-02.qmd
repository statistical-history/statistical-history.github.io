...
Parts 1b and 1c adapt Exercises 2.2 and 2.7 of Peter Hoff's *A First Course in Bayesian Statistical Methods* (2009), pp. 225–226. **Everything you need is stated below — you do not need another book.** If you want extra review of random variables, expectations, and variances, [Hoff's text](https://sites.math.rutgers.edu/~zeilberg/EM20/Hoff.pdf) §2.4 is freely available.
...
- Go to the [statistical-history](https://github.com/statistical-history) organization on GitHub. Click on the repo with the prefix `hw-02`. It contains the starter documents you need.
- Clone it the same way you cloned HW 01's repo — see [Lab 01](https://statistical-history.github.io/labs/lab-01.html) if you need the steps again.
...
1.  For independent $Y_1, Y_2$ with means $\mu_1, \mu_2$ and variances $\sigma_1^2, \sigma_2^2$, derive the mean and variance of $aY_1 + bY_2$ and of $aY_1 - bY_2$. **State where independence is needed** — one of the two derivations needs it and the other does not. You can use linearity of expectation formula. For variance derivations, use the definition of variance. In class we have seen that for general $X, Y$, $Var(X+Y) = Var(X)+Var(Y)+2Cov(X,Y)$. Do not just apply this formula.
...
3.  Now replace $Y_2$ by a **copy of** $Y_1$ — the same observation written down twice. Does averaging still reduce variation?
4.  For $N$ independent draws, give the mean and standard deviation of both the red count $X_N = Y_1 + \dots + Y_N$ and the proportion $\widehat p_N = X_N/N$. If $N$ is multiplied by **four**, what happens to each standard deviation? Connect this distinction to the scale of fluctuations in de Moivre's work (revisit page 83 and table 2.1 in Stigler).
...
2.  Generate **three independent sequences**, each with 25,550 draws. Set a seed **once, before** generating them — never inside the function, or all three sequences come out identical.
3.  For each sequence, calculate all the running proportions with `cumsum(draws) / seq_along(draws)`.
4.  Plot the three paths together. Add horizontal lines at $0.60$, $0.58$, and $0.62$. A logarithmic horizontal axis (`log = "x"`) makes the early draws visible; the template already does this.
5.  Identify the **independent unit** being simulated. What role does returning the ball after each draw play in keeping the red probability fixed and the draws independent under this model?
6.  Inspect the paths and answer two questions. **Must the distance from** $0.60$ decrease at every draw? **Does first entering the band mean a path will stay there forever?**
...
::: callout-important
## Typo in your templates

Ignore the `In SEs` column in your template. Replace it with `Expected Successes` instead.
:::
...
7.  Finally, **put the two studies side by side** in the table your template provides: $p_0$, $\epsilon$, your confirmed $N$, and the expected number of successes at that $N$. Same rule, two events — **what did changing only the rarity of the event cost you?**
...
- Access Gradescope through the menu on the [STA 119FS Canvas site](https://canvas.duke.edu/courses/84980).
